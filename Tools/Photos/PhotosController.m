...
#pragma mark - buildUI
- (void)createNav {
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc]initWithImage:[[UIImage imageNamed:@"iconback"] original] style:UIBarButtonItemStylePlain target:self action:@selector(back)];
...
- (void)creatFullUI {
    ...
    [leftButton setImage:[[UIImage imageNamed:@"iconback"] original]forState:UIControlStateNormal];
...
//选择完成后执行的方法
- (void)complete {
    [self.delegate getImagesArray:[self selectImages] indexrow:_row];
