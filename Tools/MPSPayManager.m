...
//主要支付逻辑
- (void)payWithPrice:(NSInteger)price channel:(MPSChannel)channel withId:(NSString*)str
{
...
- (void) _persistenceOrderWithStatus:(MPSPayStatus)status
{
    if (_currentCharge)
    {
        ...
        [HTTPRequest postWithURL:@"api/order/modifypaystatus" method:@"POST" params: @{@"orderId":order.orderId,@"status":@"成功"} ProgressHUD:nil controller:self response:^(id json) {
            
            NSLog(@"change order success%@",json);
