...
@interface ActivityDetailViewController ()<UIWebViewDelegate,ChooseRankDelegate>{
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    NSString *url = [NSString stringWithFormat:@"http://192.168.0.106:8888/app1.html?activityId=%@&token=%@",_str,[dic objectForKey:@"token"]];

//    NSString *url = @"http://192.168.0.106:8020/jh/index.html";
    ...
    [HTTPRequest postWithURL:@"api/activity/getproductlistforactivity" method:@"POST" params: @{@"activityId":_str} ProgressHUD:self.Hud controller:self response:^(id json) {
        ...
        typeiddict = [dic objectForKey:@"productIdArray"];
...
-(void)addGoodsCartBtnClick{
    ...
    cvc.num = number;
    cvc.type = [typeiddict objectForKey:@"typestr"];
    cvc.price = pricestr;
    cvc.str = title;
    cvc.objectId = _str;
